...
**self-contained** ONNX inference engine: no native ONNX Runtime download or runtime C++ link, so the
build stays hermetic and the dependency tree is vettable. (It builds some SIMD kernels via `cc`, so a
C toolchain is required at build time — but no prebuilt native runtime is fetched or linked.) It runs a user-supplied fixed-shape
...
- A larger (but self-contained, no native runtime) dependency tree, exempted in `cargo vet`. `multiple-versions` stays a
...
- **`ort` 2.x with `load-dynamic`** (link a system `libonnxruntime` instead of downloading one):
  ...
  deployment host. Rejected for the hermetic, self-contained option.
