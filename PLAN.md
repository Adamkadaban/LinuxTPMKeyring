...
- [x] **ONNX face matcher backend (#56):** the pure-Rust `tract` engine behind the off-by-default `face-model` feature (`mug` + forwarded by `tess-cli`); model from `MUG_MODEL_PATH`/`MugConfig.model_path` (none ships). `Matcher<Box<dyn EmbeddingExtractor>>` selects mock vs `TractExtractor` at runtime; the default/CI build stays model-free. Chosen over `ort` (whose `1.x` is yanked → fails `cargo deny`'s `yanked = "deny"`, and `2.x` is rc-only with a non-hermetic native `download-binaries`) — see ADR-0015. `cargo deny`/`cargo vet` green; CI builds + tests the feature
