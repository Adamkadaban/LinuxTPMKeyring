...
- [x] **Real Brio IR hardware capture wired into the tess face flow (#63):** `MUG_IR_BACKEND` (`auto`/`virtual`/`hardware`) selects the backend — the virtual substrate stays the CI/default path, hardware is opt-in and builds `V4l2IrDevice::open_brio()` + `BrioEmitter` (GREY IR node by-id, UVC-XU emitter), reporting cleanly unavailable (degrade to PIN) with no camera. Selection logic unit-tested with the substrate; real capture + photo-rejection is a documented **manual host smoke**, never CI (hardware never runs in CI)
- [ ] **`ort` ONNX matcher backend (#56) — BLOCKED, not landed:** no stable, non-yanked `ort` exists on crates.io (the entire `1.x` line is yanked → fails `cargo deny`'s `yanked = "deny"`; `2.x` is `rc`-only → violates the no-alpha/rc rule), and `ort` 2.x's default `download-binaries` feature fetches a prebuilt native ONNX Runtime at build time (non-hermetic, un-vettable). Trait + model-free mock retained unchanged; needs a maintainer decision before wiring
