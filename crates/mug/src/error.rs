...
pub enum MugError {
    ...
    /// No face was found in the frame (detector returned nothing above threshold). The face factor
    /// degrades to the PIN; a "no face present" frame must never reach the matcher and false-match.
