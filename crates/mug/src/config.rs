...
pub struct MugConfig {
    ...
    /// Path to the ONNX face-detector model (YuNet). `None` means no detector is configured, so the
    /// face cannot be located/aligned and the face factor degrades to the PIN — tess ships no model.
    #[serde(default)]
